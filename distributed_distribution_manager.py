...
#     def push_url_to_master(self, url, priority=10):
#         """所有 URL 统一先存入 `url_queue`，并加入优先级队列"""
#         if not self.is_visited(url):
#             self.redis_client.lpush("url_queue", url)
#             self.redis_client.zadd("url_queue_priority", {url: priority})  # 存入优先级队列
...
#         if URL_DISTRIBUTION_STRATEGY == "priority":
#             priority = self.redis_client.zscore("url_queue_priority", url) or 10
